...
#Esta versión incluye las variables manuales y binarias
...
generar_columnas_rango <- function(data, columnas) {
  # Itera sobre las columnas especificadas
  for (columna in columnas) {
    # Calcula Q1 y Q3
    Q1 <- quantile(data[[columna]], probs = 0.25)
    Q3 <- quantile(data[[columna]], probs = 0.75)
    
    # Calcula el RIC (Rango Intercuartil)
    RIC <- Q3 - Q1
    
    # Define límites para los rangos
    limite_inferior1 <- Q1 - 1.5 * RIC
    limite_inferior2 <- Q1
    limite_superior1 <- Q3
    limite_superior2 <- Q3 + 1.5 * RIC
    
    # Crea una nueva columna que represente los rangos
    nueva_columna <- paste0("c", columna, "_")
    data[, (nueva_columna) := cut(data[[columna]],
                                   breaks = c(-Inf, limite_inferior1, limite_inferior2, limite_superior1, limite_superior2, Inf),
                                   labels = c("Muy_bajo", "Bajo", "Medio", "Alto", "Muy_alto"),
                                   include.lowest = TRUE)]
    
    # Convierte las categorías en columnas binarias
    data <- dcast(data, ... ~ nueva_columna, fun.aggregate = length, fill = 0)
  }
  # Devuelve el conjunto de datos modificado
  return(data)
}

# Ejemplo de uso
# Supongamos que 'dataset' es tu data.table y 'columnas' es un vector de nombres de columnas
dataset_resultante <- generar_columnas_rango(dataset, columnas)
