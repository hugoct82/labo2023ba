...
PARAM <- list()
PARAM$experimento <- "HT6510_ep_01"

PARAM$exp_input <- "TS6410_ep_01"
...
#Donde quedan los parametros que se optimizan?
...
##--------MODIFICACIÓN DETECCIÓN DEL CAMPO B+1 y B+2------##
  tbl <- copy(dataset_test[, list("gan" = ifelse(clase_ternaria == "BAJA+2", 117000, -3000))])
  tbl <- copy(dataset_test[, list("gan" = ifelse(clase_ternaria == "BAJA+1", -3000L,
...
fganancia_lgbm_mesetaCV <- function(probs, datos) {
  ...
  #Promedio de ventana movil centrada
...
##-----MODIFICACIÓN PARA SEPARACIÓN DE B+1 y B+2----##

dataset <- dataset[clase_ternaria != "CONTINUA", ] #elimino los CONTINUA
# defino la clase binaria clase01, quiero distinguir entre BAJA+1 y BAJA+2
dataset[, clase01 := ifelse(clase_ternaria == "BAJA+1", 0L,
 ifelse(clase_ternaria == "BAJA+2", 1L, NA_integer_))]
#podria colocar un indicar extra de cliente en mal estado de salud?

##-----FIN DE MODIFICACIÓN PARA SEPARACIÓN DE B+1 y B+2----##
...
dtrain <- lgb.Dataset(
  ...
  weight = dataset[
    ...
    ifelse(clase_ternaria == "BAJA+2", 1.0000001,
      ifelse(clase_ternaria == "BAJA+1", 1.0, 0) #MODIFICO el TRAIN P/que no vea el continua, se indica #0#
    )
  ],
  free_raw_data = FALSE
)
