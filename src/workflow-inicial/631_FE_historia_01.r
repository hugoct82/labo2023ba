...
AgregaVarRandomForest <- function(
  ...
  set.seed(semilla, kind = "L'Ecuyer-CMRG") #gen de nuevos numeros aleatorios por met L'Ecuyer-CMRG
  azar <- runif(nrow(dataset_rf))

  dataset_rf[, entrenamiento :=
    as.integer(foto_mes >= 202101 & foto_mes <= 202103 & (clase01 == 1 | azar < 0.10))]

  # imputo los nulos, ya que ranger no acepta nulos
  # Leo Breiman, ¿por que le temias a los nulos?
  set.seed(semilla, kind = "L'Ecuyer-CMRG")
  dataset_rf <- na.roughfix(dataset_rf) #imputación simple de valores faltantes en el conjunto de datos dataset_rf
  ...
  #El argumento with = FALSE se utiliza para indicar que campos_buenos se interpreta como nombres de columnas, no como expresiones.
