...
# solo tiene  152 variables de las 155 originales
...
pdf("zeroes_ratio.pdf") #abrir un dispositivo gráfico en formato PDF
...
for (campo in campos_buenos) {
  ...
  #get(campo): Esta parte de la expresión utiliza
  # la función get() para obtener los valores de la
  # columna identificada por el nombre contenido en la variable campo.
...
for (campo in campos_buenos) {
  tbl <- dataset[
    ,
    list("na_ratio" = sum(is.na(get(campo)), na.rm = TRUE) / .N), # consultar si está bien colocar al final el na.rm = TRUE
    foto_mes
  ]
...
dev.off() #utiliza para cerrar un dispositivo gráfico abierto actualmente. 
...
#   tales que no son ni nulos ni tampoco valen cero
