...
require("ranger") # paquete que implementa Random Forest
...
PARAM <- list()
PARAM$experimento <- "FE6310_ep_01"

PARAM$exp_input <- "DR6210_ep_01"
...
fganancia_lgbm_meseta <- function(probs, datos) {# Consultar que es esto?
...
CanaritosAsesinos <- function(
  ...
  campos_buenos <- setdiff(
    colnames(dataset),
    c("clase_ternaria", "clase01", "foto_mes")#clase01 agrupa los baja+1 y baja+2
  )
  ...
  dataset[, entrenamiento :=
    foto_mes >= 202101 & foto_mes <= 202103 & (clase01 == 1 | azar < 0.10)]#Para que se mete azar al entrenamiento?
#y porque solo se mete en el 202101 al 202103?
  dtrain <- lgb.Dataset(
    data = data.matrix(dataset[entrenamiento == TRUE, campos_buenos, with = FALSE]),
    label = dataset[entrenamiento == TRUE, clase01],
    weight = dataset[
      entrenamiento == TRUE,
      ifelse(clase_ternaria == "BAJA+2", 1.0000001, 1.0)#para q el weight de ganancia es 1.000001?
    ],
    free_raw_data = FALSE
  )
  ...
  dvalid <- lgb.Dataset(
    ...
    free_raw_data = FALSE
  ) # porque los canaritos aplican al 202105?
  ...
  #esta parte del codigo hace el corte está 
  #seleccionando las filas que corresponden a características que tienen "canarito" en su nombre
